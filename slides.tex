...
\date{Networks 2021\\{\footnotesize(virtual)}\\July 10, 2021\vspace{-1cm}}
...
\begin{frame}[c,label=geneontology]
	...
	\begin{minipage}[m]{.66\linewidth}
		...
		\end{figure}
		\begin{itemize}
			\item $\sim$ 15,000 phylogenetic trees		
			\item $\sim$ 8 million annotations
			\item $\sim$ 600 thousand on human genes
			\item $\sim$ $<$ 10\% are based on experimental evidence... \uncover<6->{Improving our knowledge on genetics is fundamental for advancing Biomedical Research}
		\end{itemize}
	\end{minipage}
	\vfill \hfill
	\Large \uncover<1->{Only on 2020, 2,000+ COVID-19 papers using the GO \href{https://scholar.google.com/scholar?as\_ylo=2020\&q="gene+ontology"+(covid+OR+coronavirus)}{(Google Scholar)}}\normalsize\\
...
\begin{frame}
	...
	\begin{figure}
		\scalebox{.8}{
			...
			\mode<beamer>{%
				...
				\only<1->{\input{fig/polytomies-2.pdf_tex}}%
				\only<1->{\input{fig/polytomies.pdf_tex}}%
			}
		}
	...
	\end{figure}
	\uncover<1->{\small(in practice, arrays up to 32 cells, i.e., 4.3 billion comb., are feasible.)}
\end{frame}
...
\begin{frame}
	\begin{figure}
		\includegraphics[width=.45\linewidth]{mcmc-analysis.pdf}
		\caption{\textit{Receiver Operating Characteristic Curve}. Comparing each set of genes from the 77 trees totaling 709 present/absent annotations in 77 phylogenetic trees. \textbf{GEESE} outperforms \textbf{aphylo} in both MAE and AUC.}
	\end{figure}
\end{frame}
...
\begin{frame}[label=geese-vs-aphylo2,c]
	\frametitle{Analyzing 70 experimentally annotated trees (Flock)}
	\begin{minipage}[m]{.44\linewidth}
	Same as before, with the exception of:
	\begin{itemize}
		\item<2-> 70 experimentally annotated trees
		\item<3-> Reduced the set to trees with a single function
		\item<4-> A single pooled-model with \textbf{\large$\approx$34,000 arrays}.
		\item<5-> Largest polytomy 27, meaning about \textbf{\large 250,000,000 combinations}.
		\item<6-> Fitted within 15 minutes
	\end{itemize}
	\end{minipage}%
	\hfill
	\begin{minipage}[m]{.55\linewidth}
		\uncover<7->{
\begin{tabular}{m{.6\linewidth}rl}
	\toprule
	& \multicolumn{2}{c}{MCMC est.} \\\midrule
	\# of genes changing at dupl & -1.85 & (0.05)\\
	Gains at dupl. & -1.14 & (4.30)\\
	Gains at spec. & -2.50 & (0.09)\\
	Loss at spec. & -3.70 & (0.07)\\
	Root has the fun. & 5.83 & (3.78)\\\bottomrule
\end{tabular}}
	\end{minipage}
\end{frame}
...
\begin{minipage}[m]{.49\linewidth}
	\centering
	\large{\bfseries Transition Probabilities \vphantom{(``Gibbs'')}} \normalsize \\
	\vspace{.5cm}
	\includegraphics[width=.95\linewidth]{transition-conditional-prob-transition.png}\\
	\vspace{.5cm}
	\uncover<2->{\begin{tabular}{ccc} \toprule %
		\textbf{Event type} & \textbf{Duplication} & \textbf{Speciation} \\ \cmidrule(r){2-3} 
		\textbf{Both gain} & 0.02 & \hphantom{$<$}0.01 \\
		\textbf{Both lose} & 0.02 & $<$0.01 \\ \bottomrule
	\end{tabular}}
%	\caption{Transition Probabilities: Siblings negate parent}
\end{minipage} %
\hfill %
\begin{minipage}[m]{.49\linewidth}
	\centering
	\uncover<3->{\large{\bfseries Conditional Probabilities (``Gibbs'')} \normalsize \\
	\vspace{.5cm}
	\includegraphics[width=.95\linewidth]{transition-conditional-prob-conditional.png}}\\
	\vspace{.5cm}
	\uncover<4->{
	\begin{tabular}{ccc} \toprule %
	\textbf{Event type} & \textbf{Duplication} & \textbf{Speciation} \\ \cmidrule(r){2-3} 
	\textbf{Preserve 0} & 0.85 & 0.90 \\
	\textbf{Preserve 1} & 0.86 & 0.97 \\ \bottomrule
	\end{tabular}}
%	\caption{Conditional Probabilities: Preserve parent given sibling does}
\end{minipage}
...
\begin{frame}[t]
	...
	\uncover<3->{\byside{.33}{Today}{
			...
			\begin{itemize}
				...
				\item \textbf{geese} (ERGMs): \textbf{GE}ne function \textbf{E}volution using \textbf{S}uffici\textbf{E}ncy.
			\end{itemize}
	}}\hfill
	\uncover<4->{\byside{.33}{Next steps}{
			...
			\begin{itemize}
				...
				\item Find applications for this \sout{model} \textbf{modeling framework}.
			\end{itemize}
	}}
\end{frame}
